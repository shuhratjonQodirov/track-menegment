...
public interface TripRepository extends JpaRepository<Trip, Long> {
    ...
    Long countByTripStatusAndDeletedFalse(TripStatus status);

    Long countByStartedDateAndDeletedFalse(LocalDate date);

    Long countByTripStatusAndFinishedDateBetweenAndDeletedFalse(
            TripStatus status, LocalDate start, LocalDate end
    );

    List<Trip> findByFinishedDateBetweenAndDeletedFalse(LocalDate start, LocalDate end);

    List<Trip> findByDriverIncomePaidFalseAndDeletedFalse();

    List<Trip> findByDriverIncomePaidTrueAndFinishedDateBetweenAndDeletedFalse(
            LocalDate start, LocalDate end
    );

    List<Trip> findByUserAndTripStatusAndDeletedFalse(User user, TripStatus status);

    List<Trip> findTop5ByOrderByStartedDateDesc();

    Optional<Trip> findByTruckAndDeletedFalseAndDeletedFalse(Truck truck);
